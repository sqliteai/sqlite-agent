...
@main
struct agent: BuildToolPlugin {
    /// Entry point for creating build commands for targets in Swift packages.
    func createBuildCommands(context: PluginContext, target: Target) async throws -> [Command] {
        let packageDirectory = context.package.directoryURL
        let outputDirectory = context.pluginWorkDirectoryURL
        return createagentBuildCommands(packageDirectory: packageDirectory, outputDirectory: outputDirectory)
    }
}
...
extension agent: XcodeBuildToolPlugin {
    // Entry point for creating build commands for targets in Xcode projects.
    func createBuildCommands(context: XcodePluginContext, target: XcodeTarget) throws -> [Command] {
        let outputDirectory = context.pluginWorkDirectoryURL
        return createagentBuildCommands(packageDirectory: nil, outputDirectory: outputDirectory)
    }
}

#endif

/// Shared function to create agent build commands
func createagentBuildCommands(packageDirectory: URL?, outputDirectory: URL) -> [Command] {
    ...
    return [
        .prebuildCommand(
            displayName: "Building agent XCFramework",
            executable: URL(fileURLWithPath: "/bin/bash"),
            arguments: [
                "-c",
                """
                set -e
                echo "Starting agent XCFramework prebuild..."
                echo "\(packageDirInfo)"
