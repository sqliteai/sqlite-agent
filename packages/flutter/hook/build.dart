...
void main(List<String> args) async {
  await build(args, (input, output) async {
    ...
    if (binaryPath == null) {
      throw UnsupportedError('sqlite_agent does not support $os $arch.');
    }
    ...
    output.assets.code.add(
      CodeAsset(
        ...
        name: 'src/native/sqlite_agent_extension.dart',
...
Future<File> _prepareAssetFile({
  ...
  final outputName = 'agent_ios_sim_$thinArch.dylib';
  ...
  if (result.exitCode != 0) {
    throw StateError(
      'Failed to thin sqlite_agent iOS simulator binary for $thinArch: '
      '${result.stderr}',
    );
  }
...
String? _resolveBinaryPath(OS os, Architecture arch, CodeConfig config) {
  if (os == OS.android) {
    return switch (arch) {
      Architecture.arm64 => 'android/agent_android_arm64.so',
      Architecture.arm => 'android/agent_android_arm.so',
      Architecture.x64 => 'android/agent_android_x64.so',
      _ => null,
    };
  }
  ...
  if (os == OS.iOS) {
    ...
    if (sdk == IOSSdk.iPhoneOS) {
      return 'ios/agent_ios_arm64.dylib';
    }
    // Simulator: fat binary (arm64 + x64)
    return 'ios-sim/agent_ios-sim.dylib';
  }

  if (os == OS.macOS) {
    return switch (arch) {
      Architecture.arm64 => 'mac/agent_mac_arm64.dylib',
      Architecture.x64 => 'mac/agent_mac_x64.dylib',
      _ => null,
    };
  }
  ...
  if (os == OS.linux) {
    return switch (arch) {
      Architecture.x64 => 'linux/agent_linux_x64.so',
      Architecture.arm64 => 'linux/agent_linux_arm64.so',
      _ => null,
    };
  }
  ...
  if (os == OS.windows) {
    return switch (arch) {
      Architecture.x64 => 'windows/agent_windows_x64.dll',
      _ => null,
    };
  }
